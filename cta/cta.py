...
class DefillamaResearchEngine(ResearchEngine):
    ...
    def read_data(dirpath, start_date, selected_instruments: list[Instrument]) -> FileData:
        ...
        for filename in os.listdir(dirname):
            ...
            if Instrument(instrument) in selected_instruments:
                new_df = pd.read_csv(os.path.join(os.sep, dirname, filename))
                new_df['time'] = new_df['time'].apply(lambda t: datetime.fromtimestamp(t / 1000))
                new_df = new_df.set_index('time').sort_index(ascending=True)[start_date:]
                if instrument in file_data and data_type in file_data[instrument]:
                    file_data[instrument][data_type] = pd.concat([file_data[instrument][data_type], new_df])
                else:
                    if instrument in file_data:
...
def build_ResearchEgine(parameters) -> ResearchEngine:
    ...
    file_data = BinanceExtremeResearchEngine.read_data(**parameters['input_data'])
